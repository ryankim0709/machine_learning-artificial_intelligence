...
# We will use mean normalization in order to decrease this number

norm_train_df = exo_train_df.iloc[:,1:].apply(mean_normalization, axis=1)
print(norm_train_df.head())
print(norm_train_df.describe())
# As you can see, the difference is much smaller now which means our normalization has worked!

# We normalization just the flux's, so we must put the labels back
